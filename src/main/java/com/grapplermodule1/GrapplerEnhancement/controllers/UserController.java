...
//                List<UsersDTO> usersPage = userService.fetchAllUsersByPagination(PageRequest.of(page, size));
                List<UsersDTO> usersList = userService.fetchAllUsers();
                return new ResponseEntity<>(usersList, HttpStatus.OK);
            } else {
                List<UsersDTO> usersList = userService.fetchAllUsers();
...
//    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping("/")
    // @Valid
    public ResponseEntity<UsersDTO> createUser(@RequestBody UserPayload user) {
        String debugUuid = UUID.randomUUID().toString();
        try {
...
//    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    @GetMapping("/{userId}")
    public ResponseEntity<UsersDTO> getUserById(@Valid @PathVariable("userId") Long userId) {
        String debugUuid = UUID.randomUUID().toString();
        try {
...
//    @PreAuthorize("hasAnyRole('ADMIN', 'USER')")
    @PutMapping("/{userId}")
    public ResponseEntity<UsersDTO> updateUserById(@PathVariable("userId") Long userId,
                                            @Validated(PutValidation.class) @RequestBody UsersDTO user) {
        String debugUuid = UUID.randomUUID().toString();
        try {
            log.info("UUID {} Inside Update User By Id, User Id {} ", debugUuid, userId);
            UsersDTO updatedUser = userService.updateUserDetails(userId, user);

            log.info("Update User By Id Returning User in ResponseEntity, Updated User Id {} ", updatedUser);
