...
import com.grapplermodule1.GrapplerEnhancement.customexception.LeafFolderException;
import com.grapplermodule1.GrapplerEnhancement.customexception.ParentNotFoundException;
...
import com.grapplermodule1.GrapplerEnhancement.service.FolderService;
...
public class FolderController {
    ...
    public ResponseEntity<ApiResponseProject<List<Folder>>> getAllFolders(@PathVariable Long projectId) {
        try {
            ...
            if (!folders.isEmpty()) {
                ...
                return ResponseEntity.ok(new ApiResponseProject<>(true, folders, "All folders retrieved successfully"));
            } else {
                logger.warn("No folders found for Project ID: {}", projectId);
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponseProject<>(false, null, "No folders found  with Project ID : "+projectId));
            }
    ...
    public ResponseEntity<ApiResponseProject<Folder>> getFolder(@PathVariable Long folderId) {
        try {
            Folder folder = folderService.getFolder(folderId);
            if (folder != null) {
                logger.info("Folder retrieved successfully with ID: {}", folderId);
                return ResponseEntity.ok(new ApiResponseProject<>(true, folder, "Folder retrieved successfully"));
            } else {
                logger.warn("Folder not found with Folder ID: {}", folderId);
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponseProject<>(false, null, "Folder not found with ID : "+folderId));
            }
        } catch (Exception e) {
    ...
    public ResponseEntity<ApiResponseProject<Folder>> createFolder(@PathVariable Long projectId, @Valid @RequestBody Folder folder) {
        ...
        try {
            Folder createdFolder = folderService.createFolder(projectId, folder);
                ...
                return ResponseEntity.status(HttpStatus.CREATED).body(new ApiResponseProject<>(true, createdFolder, "Folder created successfully"));
        } catch (ParentNotFoundException ex) {
            logger.error("Parent not found : {}", ex.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponseProject<>(false, null, ex.getMessage()));
        }  catch (UnsupportedOperationException ex) {
            logger.error(ex.getMessage());
            return ResponseEntity.status(HttpStatus.UNPROCESSABLE_ENTITY).body(new ApiResponseProject<>(false, null, ex.getMessage()));
        }catch (Exception e) {
    ...
    public ResponseEntity<ApiResponseProject<Folder>> createSubFolder(@PathVariable Long parentFolderId, @Valid @RequestBody Folder folder) {
        try {
...
//    }
    ...
    public ResponseEntity<ApiResponseProject<Folder>> updateFolder( @PathVariable Long folderId,@Valid @RequestBody Folder folder) {
        try {
            ...
            if (updatedFolder != null) {
                ...
                return ResponseEntity.ok(new ApiResponseProject<>(true, updatedFolder, "Folder updated successfully with ID : "+folderId));
            } else {
                logger.warn("Folder not found with Folder ID: {}", folderId);
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ApiResponseProject<>(false, null, "Folder not found with ID : "+folderId));
            }
    ...
    public ResponseEntity<ApiResponseProject<?>> deleteFolder(@PathVariable Long folderId) {
        try {
