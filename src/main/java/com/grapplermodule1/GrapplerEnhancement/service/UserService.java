...
import com.grapplermodule1.GrapplerEnhancement.dto.UserPayload;
...
//    TicketRepository ticketRepository;
     ...
     * @return Users
     */
    public UsersDTO addUser(UserPayload user) {
        try {

            Users actualUser = new Users();

            actualUser.setName(user.getName());
            actualUser.setDesignation(user.getDesignation());
            actualUser.setEmail(user.getEmail());
            actualUser.setRole(user.getRole());
            Optional<Users> subordinate = userRepository.findById(user.getReportingUserId());
            if(subordinate.isPresent()){
                actualUser.setReportingUser(subordinate.get());
            }

            log.info("Add New User Service Called");
            Optional<Users> existingUser = userRepository.findByEmail(actualUser.getEmail());
            ...
            Users newUser = userRepository.save(actualUser);

            Role role = new Role();
            role.setRole(actualUser.getRole().getRole());
     ...
     * @return Users
     */
    public UsersDTO updateUserDetails(Long userId, UsersDTO user) {
        try {
            ...
            if (existingOptionalUser.isPresent()) {
                ...
                usersDTO.setId(existingUser.getId());
                usersDTO.setName(existingUser.getName());
                usersDTO.setDesignation(existingUser.getDesignation());
                usersDTO.setEmail(existingUser.getEmail());
